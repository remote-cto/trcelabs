...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const interactiveElements = document.querySelectorAll(
    "a, button, .service-card, .dashboard-mockup"
  );
  ...
  interactiveElements.forEach((el) => {
    el.addEventListener("mouseenter", () => {
      ...
      cursorGlow.style.background =
        "radial-gradient(circle, rgba(0, 240, 255, 0.08) 0%, rgba(0, 0, 0, 0) 60%)";
    });
    ...
    el.addEventListener("mouseleave", () => {
      ...
      cursorGlow.style.background =
        "radial-gradient(circle, rgba(0, 240, 255, 0.06) 0%, rgba(0, 0, 0, 0) 70%)";
    });
  });
...
const securityBtn = document.getElementById("openSecurity");
securityBtn.onclick = function (e) {
  e.preventDefault();
